...
```bash
nextflow run nf-core/hmfrnaseq -profile docker -params-file params.yaml
```
...
```bash
nextflow pull nf-core/hmfrnaseq
```
...
First, go to the [nf-core/hmfrnaseq releases page](https://github.com/nf-core/hmfrnaseq/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
